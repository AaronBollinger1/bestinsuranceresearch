...
- `/ask` uses `FocusPageHeader`; sole primary submit is **Ask a question**; **Get cited** is quiet.
- Featured examples stay one hop to `/questions/<id>` (no JS trap).
- Lookup and browse rows name the next step: Read the answer / Open sources on the record.
